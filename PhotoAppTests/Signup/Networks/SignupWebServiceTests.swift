...
class SignupWebServiceTests: XCTestCase {
    ...
    func test_signupWebService_WhenReceivedDifferentJSONResponse_ErrorTookPlace() {
        ...
        sut.signup(withForm: signFormRequestModel) { (signupResponseModel, error) in
            ...
            // Assert
            XCTAssertNil(signupResponseModel, "The response model for a request containing unknown JSON response, should have benn nil")
            XCTAssertEqual(error, SignupErrors.invalidResponseModel, "The signup() method did not return expected error")
            expectation.fulfill()
    ...
    func test_signupWerbService_WhenEmptyURLStringProvided_ReturnError() {
        // Arrange
    ...
    func test_signupWebService_WhenURLRequestFails_ReturnsErrorMessageDescription() {
        ...
        let errorDescription = "The operation couldn’t be completed. (PhotoApp.SignupErrors error 0.)"
